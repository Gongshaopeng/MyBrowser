...
@implementation GSLAlertView

+(instancetype)alertManager{
    static GSLAlertView *manager=nil;
    static dispatch_once_t once;
    dispatch_once(&once ,^
                  {
                      if (manager==nil)
                      {
                          manager=[[GSLAlertView alloc]init];
                      }
                  });
    return manager;
}
...
{
     if (self == [super init]) {
         [self createInitWithTitle:title message:message sureBtn:sureTitle cancleBtn:cancleTitle];
     }
