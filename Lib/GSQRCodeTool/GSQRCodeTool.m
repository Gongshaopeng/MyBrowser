...
#import "GSQRCodeTool.h"

@implementation GSQRCodeTool
/**
 *  生成一张普通的二维码
 *
...
{
    if (size < 0) {
        return  [self SG_generateWithLogoQRCodeData:[self decodeFromPercentEscapeString:url]?:@"" logoImageName:@"about_bird_1" logoScaleToSuperView:0.2];
    }else{
        NSString * strUrl = url;
        
//                if ([GSRegular justWithInitRegularly:[HistoryData Scoredomain] Str:strUrl]) {
//                    NSArray *temp=[url componentsSeparatedByString:@"?"];
//                    if (temp.count != 0) {
//                        strUrl = temp[0];
//                    }
//                }
        return [self SG_generateWithDefaultQRCodeData:[self decodeFromPercentEscapeString:strUrl]?:@"" imageViewWidth:size];
    }
